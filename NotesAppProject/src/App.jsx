import { useState } from "react";
// import { X } from 'lucide-react';

function App() {
  const [title, setTitle] = useState("");
  const [details, setDetails] = useState("");
  const [task, setTask] = useState([]);
  const submitHandler = (e) => {
    e.preventDefault();
    const copyTask = [...task];
    copyTask.push({ title, details });
    setTask(copyTask);
    console.log(task);

    setTitle("");
    setDetails("");
  };

  const deleteNote = (idx) => {
    const copyTask = [...task];
    copyTask.splice(idx, 1);

    setTask(copyTask);
  };
  return (
    <div className="h-screen bg-black text-white lg:flex">
      <form
        onSubmit={submitHandler}
        className="lg:w-1/2 flex flex-col items-start gap-5 p-10"
      >
        <h1 className="text-4xl font-bold">Add Notes</h1>
        <input
          type="text"
          placeholder="Enter Notes Heading"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          className=" w-full px-5 py-2 border-2 rounded outline-none font-medium"
        />
        <textarea
          name=""
          id=""
          placeholder="Write details here"
          value={details}
          onChange={(e) => setDetails(e.target.value)}
          className="h-32 w-full px-5 py-2 border-2 rounded outline-none font-medium"
        ></textarea>
        <button className="bg-white active:scale-110 text-black w-full px-5 py-2 rounded outline-none font-medium cursor-pointer">
          Add Note
        </button>
      </form>
      <div className="lg:w-1/2 lg:border-l-2 p-10">
        <h1 className="text-4xl font-bold">Recent Notes</h1>
        <div className="h-[90%] flex flex-wrap justify-start items-start gap-5 mt-5 overflow-auto">
          {task.map((obj, idx) => {
            return (
              <div
                key={idx}
                className="flex flex-col justify-between items-start h-52 w-40 rounded-2xl text-black pt-9 pb-5 px-4 bg-white bg-[url('data:image/png;base64,')] bg-cover"
              >
                {/* <h1 className="absolute top-5 right-5 bg-red-500 text-xs p-1 rounded-full"><X size = {16} color="ffffff" strokeWidth={2.75} /></h1> */}
                <div>
                  <h3 className="leading-tight text-lg font-bold">
                    {obj.title}
                  </h3>
                  <p className="mt-2 leading-tight text-xs font-semibold text-gray-600">
                    {obj.details}
                  </p>
                </div>
                <button
                  onClick={() => {
                    deleteNote(idx);
                  }}
                  className="w-full py-1 text-xs bg-red-400 text-white font-bold rounded cursor-pointer active:scale-95"
                >
                  Delete note
                </button>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
}

export default App;
